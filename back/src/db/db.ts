...
import { findAndRemoveElement, Game, GameSettings, GameState, removeElement, User } from 'back-common'
...
export class DB {
  ...
  async init() {
    ...
    this.gamesSettings = await JSONFilePreset<GameSettings[]>(path.join(dbDir, 'games-settings.json'), [])
    this.gamesStates = await JSONFilePreset<GameState[]>(path.join(dbDir, 'games-states.json'), [])
  }
