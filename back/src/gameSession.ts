import { Connection, DataMessageListener, Game, GameActionMessage, GameBackService, GameContext, GamePublicState, GameSettings, GameState, GameStatusEnum, getSubObjectChanges, handleMessage, JoinGameMessage, KickPlayerMessage, MesasgeHandlers, NotifyGameMessage, ObjectSync, PeerFilter, PlayerPrivateState, PropChange, removeElement, StartGameMessage, TypedMessage, UpdateUserRequest, watchChagesList } from "back-common";
...
export class GameSession implements Connection {
    ...
    addConnection(connection: WsConnection) {
        ...
        if (this.gameState) {
            ...
            this.playerPrivateStateSync?.get(connection.id())
        ...
        connection.webSocket.on("message", (messageString: string) => {
            ...
            type messageTypes = GameActionMessage |
                JoinGameMessage |
                UpdateUserRequest |
                StartGameMessage |
                KickPlayerMessage
            ...
            const handlers: MesasgeHandlers<messageTypes> = {
                ...
                StartGameMessage: async (message: StartGameMessage) => {
                ...
                JoinGameMessage: async (_message: JoinGameMessage) => {
                ...
                GameActionMessage: async (message: GameActionMessage) => {
                    ...
                    const gameStateProxy = this.gameState ? watchChagesList(this.gameState, gameStateChanges) : undefined
                    ...
                    await this.gameService.performAction({
                        ...
                        sendNotify: (peerId: string | undefined, message: string, messageParams: any | undefined) => this.sendNotify(peerId, message, messageParams)
                    }, message.action, connection.id())

                    if (this.gameState) {
                        const publicStateChanges = getSubObjectChanges(gameStateChanges, ['publicState'])
                        ...
                        const privatePlayerStatesChanges = getSubObjectChanges(gameStateChanges, ['privateState', 'playersStates'])
                        const playerChangesMap = new Map<number, PropChange[]>()

                        for (var change of privatePlayerStatesChanges) {
        ...
        connection.webSocket.on("close", (scocket) => {
            this.connections.delete(connection.id())
            const playerIndex = this.game.players.findIndex(player => player.userId == connection.id());
